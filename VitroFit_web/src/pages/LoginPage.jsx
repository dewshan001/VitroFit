...
import { login, verifyEmail, resendVerification } from '../api/auth';
            ...
            {!verifyMode ? (
              ...
              <>
                ...
                <div className="auth-card-header">
                  ...
                  <p className="section-label">Verify Your Email</p>
                  <h2 className="auth-card-title">Check Your Inbox</h2>
                  <p className="auth-card-sub">
                    We sent a 6-digit code to <strong style={{ color: 'var(--accent, #c8f000)' }}>{form.email}</strong>.
                    Enter it below to verify and sign in.
                  </p>
                </div>

                <form className="auth-form" onSubmit={handleVerifyOTP} noValidate>
                  <div className={`auth-field ${errors.otp ? 'auth-field--error' : ''} ${otp ? 'auth-field--filled' : ''}`}>
                    <label htmlFor="login-otp" className="auth-label">Verification Code</label>
                    <div className="auth-input-wrap">
                      ...
                      <input
                        ...
                        className="auth-input"
                        value={otp}
                        onChange={e => { setOtp(e.target.value.replace(/\D/g, '').slice(0, 6)); if (errors.otp) setErrors({}); if (serverError) setServerError(''); }}
                        placeholder="123456"
                        maxLength={6}
                  ...
                  <button id="login-verify-submit" type="submit" className={`btn-primary auth-submit ${loading ? 'loading' : ''}`} disabled={loading || otp.length < 6}>
                    {loading
                      ? <span className="auth-spinner" />
                      : <>Verify &amp; Sign In <span className="btn-arrow">→</span></>
                    }
                  </button>
                  ...
                  {serverError && (
                    <div
                      ...
                      style={serverError.startsWith('✓') ? { backgroundColor: 'rgba(0, 255, 136, 0.1)', color: '#00ff88', border: '1px solid rgba(0, 255, 136, 0.2)' } : {}}
                    >
                      {serverError}
                    </div>
                  )}
                ...
                <p className="auth-card-switch" style={{ textAlign: 'center', marginTop: '16px' }}>
                  <button
                    type="button"
                    onClick={() => { setVerifyMode(false); setOtp(''); setServerError(''); setErrors({}); }}
                    className="auth-link"
                    style={{ background: 'none', border: 'none', padding: 0, font: 'inherit', cursor: 'pointer' }}
                  >
                    ← Back to Sign In
